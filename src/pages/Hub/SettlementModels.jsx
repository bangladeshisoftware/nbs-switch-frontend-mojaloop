...
const OPTIONS = {
  ...
  currencies: ['BDT', 'USD', 'EUR', 'INR', 'TZS'],
  ledgerAccountTypes: [
    {
      value: 'HUB_MULTILATERAL_SETTLEMENT',
      label: 'Hub Multilateral Settlement',
    },
...
export default function SettlementModels() {
    ...
    try {
      await api.post('/hub/settlement-models', {
        ...
        requireLiquidityCheck:
          form.requireLiquidityCheck === true ||
          form.requireLiquidityCheck === 'true',
        autoPositionReset:
          form.autoPositionReset === true || form.autoPositionReset === 'true',
      });
      setModal(false);
      setForm(EMPTY_FORM);
      load();
      ...
      setError(
        err.response?.data?.errorInformation?.errorDescription ||
  ...
  const fBool = (k) => (e) =>
    setForm((p) => ({ ...p, [k]: e.target.value === 'true' }));
      ...
      <div className='page-header'>
        <div>
          ...
          <div className='page-subtitle'>
            {models.length} models configured in Mojaloop Hub
          </div>
        </div>
                  ...
                  <td colSpan={11}>
                    <div className='empty-state'>
                      <div className='empty-title'>
                        No settlement models found
                      </div>
                      <div className='empty-desc'>
                        Click "+ Create Model" to add one
                      </div>
                    </div>
                  </td>
                </tr>
                ...
                models.map((m, i) => (
                  <tr key={m.settlementModelId ?? i}>
                    ...
                    <td
                      style={{ fontSize: 11, color: 'var(--text-secondary)' }}
                    >
                      {m.settlementAccountType || '—'}
                    </td>
                    <td>
                      <span
                        className={`badge ${m.requireLiquidityCheck ? 'COMMITTED' : 'TIMEOUT'}`}
                      >
                    ...
                    <td>
                      <span
                        className={`badge ${m.autoPositionReset ? 'COMMITTED' : 'TIMEOUT'}`}
                      >
                    ...
                    <td>
                      <span
                        className={`badge ${m.isActive ? 'COMMITTED' : 'FAILED'}`}
                      >
          ...
          >
            ...
            >
              ...
              >
                <div className='input-group'>
                  ...
                  <select
                    ...
                    value={form.settlementGranularity}
                    onChange={f('settlementGranularity')}
                  >
                    {OPTIONS.granularity.map((o) => (
                ...
                <div className='input-group'>
                  ...
                  <select
                    className='select w-full'
                    value={form.settlementInterchange}
                    onChange={f('settlementInterchange')}
                  >
                    {OPTIONS.interchange.map((o) => (
                      <option key={o} value={o}>
                        {o}
                      </option>
                    ))}
                ...
                <div className='input-group'>
                  ...
                  <select
                    className='select w-full'
                    value={form.settlementDelay}
                    onChange={f('settlementDelay')}
                  >
                    {OPTIONS.delay.map((o) => (
                      <option key={o} value={o}>
                        {o}
              ...
              <div className='input-group'>
                ...
                <select
                  className='select w-full'
                  value={form.ledgerAccountType}
                  onChange={f('ledgerAccountType')}
                >
                  {OPTIONS.ledgerAccountTypes.map((o) => (
              ...
              <div className='input-group'>
                ...
                <select
                  ...
                  onChange={f('settlementAccountType')}
                >
                  {OPTIONS.settlementAccountTypes.map((o) => (
                    <option key={o.value} value={o.value}>
                      {o.label}
                    </option>
                  ))}
              ...
              >
                <div className='input-group'>
                  ...
                  <select
                    className='select w-full'
                    value={String(form.requireLiquidityCheck)}
                    onChange={fBool('requireLiquidityCheck')}
                  >
                ...
                <div className='input-group'>
                  ...
                  <select
                    className='select w-full'
                    value={String(form.autoPositionReset)}
                    onChange={fBool('autoPositionReset')}
                  >
                    <option value='true'>Yes</option>
                    <option value='false'>No</option>
                  </select>
